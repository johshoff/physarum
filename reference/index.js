...
function sim_step(agents, trail, width, height) {
  ...
  function step_sense_and_rotate() {
    for (let agent of agents) {
      function sense_relative_angle(theta) {
        return trail[
          index(
            Math.round(
              agent.x +
                Math.cos(agent.heading + theta) * settings.sensor_distance
            ),
            Math.round(
              agent.y +
                Math.sin(agent.heading + theta) * settings.sensor_distance
            )
          )
        ];
      }
      ...
      const modified_turning =
        (settings.random_turning ? Math.random() * 0.5 + 0.5 : 1) *
      ...
      if (sense_middle > sense_left && sense_middle > sense_right) {
        ...
        option = 3;
        agent.heading +=
          Math.round(Math.random() * 2 - 1) * settings.turning_speed;
      }
      counts[option] += 1;
    ...
    for (let y = 1; y < height - 1; ++y) {
      for (let x = 1; x < width - 1; ++x) {
        ...
        trail[index(x, y)] = Math.min(
          1.0,
          diffused_value * settings.decay_factor
        );
      }
    }
  }
...
onload = function () {
  ...
  function next_frame() {
    ...
    document.getElementById("debug").style.display = settings.show_debug
      ? "block"
      : "none";
